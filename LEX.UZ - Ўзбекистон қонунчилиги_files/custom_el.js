...
function initCustomForms() {
    //alert('initCustomForms');
	$('select').customSelect();
	$('input:radio').customRadio();
	$('div#left_block input:checkbox').customCheckbox();
}
		...
		return this.each(function() {
			...
			if(!select.hasClass('outtaHere') && !select.hasClass(_options.filterClass)) {
				if(select.is(':visible')) {
                    ...
                    var selSTR = replaced.find('span.center');
					...
					if(copyClass){
                        ...
                        //var clname = optHolder.attr('class');
                        //selSTR.attr('onclick','showOpt(this, \''+clname+'\')');
                    }
